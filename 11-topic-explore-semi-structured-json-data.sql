...
-- 11.0.0  TOPIC: Explore Semi-Structured JSON Data
...
--         structures.
--         You should use the code file for this lab that was provided at the
--         start of the class. To use the file, simply drag and drop it into an
--         open worksheet. It is not recommended that you copy & paste from the
--         workbook PDFs as that sometimes results in errors. Text in PDFs often
--         contains hidden (invisible) characters that, when pasted into a
--         worksheet, can cause the compilation of a query to fail.

-- 11.1.0  Review the Weather Data

-- 11.1.1  Create a new worksheet and name it Explore Weather Data.

-- 11.1.2  Set your worksheet context as follows:

-- 11.1.3  Execute the following commands within your worksheet:

USE ROLE TRAINING_ROLE;
CREATE WAREHOUSE IF NOT EXISTS TOUCAN_WH;
ALTER WAREHOUSE TOUCAN_WH SET 
...
-- 11.1.4  Use the DESCRIBE TABLE command to describe the isd_2019_daily table:
...
-- 11.1.5  Select all columns from the isd_2019_daily table, limit the results
...
-- 11.1.6  In the **Results* section below the worksheet, click on the JSON data
--         to pull up the details pane with the VARIANT data.
--         This will display the structure of a single record of the JSON data.

-- 11.1.7  Use FLATTEN to extract the top-level keys from the V column. Use
--         LIMIT 10:
...
-- 11.1.8  Run a query to extract the time, station, country, elevation, temp,
...
-- 11.1.9  Run the following to display the types of columns from the last run
--         query:
...
-- 11.1.10 Return to the previous instance as that data pertains more to what we
...
-- 11.1.12 Remove the double quotes around the values by casting each value as a
...
-- 11.1.13 Run the following to display the types of columns from the last run
--         query:
...
-- 11.2.0  Extract and Transform the Data

-- 11.2.1  Extract the weather station reported air temperature data. Instead of
...
-- 11.2.3  Extract the weather station reported air temperature data that has
--         pass all quality control checks. Rename the column to temp_celsius
--         and include only temperature data with a temp-quality-code equal to
--         '1':
...
-- 11.2.4  Filter the results to records for a 7 day period starting on
...
-- 11.2.5  Write a query to pull country, station name, and highest temperatures
...
-- 11.2.6  Run the same query, but return the temperature in Fahrenheit instead
--         of Celsius.
...
-- 11.2.7  Run the following command, then click on one of the values in the V
...
-- 11.2.8  Run a query to return 10 rows showing the timestamp, the station
...
-- 11.2.9  Click on a row in the data column, to see the structure of the data
--         VARIANT. What additional information can you extract from the JSON
--         data provided?

-- 11.2.10 Use the FLATTEN table function (with a LATERAL join) to iterate
...
-- 11.2.11 Run a query to calculate the average, max and min air temperature in
--         Celsius and Fahrenheit for each weather station.

-- 11.2.12 Run a query to calculate the average, max and min air temperature in
--         Celsius and Fahrenheit for each weather station. Filter the results
--         to records for a 7 day period starting on 2020-08-14 and ending on

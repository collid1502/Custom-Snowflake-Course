...
-- 2.2.2   Locate and click TOUCAN_DB in the database list.
...
-- 2.2.9   Verify that it automatically sets the context to your standard
--         context.

-- 2.2.10  Create a schema in your database, and name it TOUCAN_SCHEMA.

-- 2.2.11  Specify the full database.schema path when you create it, then verify
--         that it is set as the default schema in your worksheet.
--         HINT: Review the syntax for creating a database; the syntax for a
--         schema is similar.

CREATE SCHEMA TOUCAN_DB.TOUCAN_SCHEMA;


-- 2.2.12  Create a MEMBERS table.

-- 2.2.13  Give it columns to hold a numeric customer ID, a first name (up to 20
...
-- 2.2.14  Query the table to make sure the columns are all there.
...
-- 2.5.2   Using SQL, create a warehouse named TOUCAN_WH2, with the following
--         characteristics.
...
-- 2.5.15  Locate TOUCAN_WH2 and verify the Status is now Started.
...
-- 2.5.17  Suspend TOUCAN_WH2:

ALTER WAREHOUSE TOUCAN_WH2 SUSPEND;
...
-- 2.5.19  Alter TOUCAN_WH2 and set the WAREHOUSE_SIZE to XSmall:

ALTER WAREHOUSE TOUCAN_WH2 SET WAREHOUSE_SIZE=XSmall;


-- 2.5.20  Suspend and then drop TOUCAN_WH2:
...
--         ’TOUCAN_WH2’ cannot be suspended."
--         Proceed with the statement below to drop the warehouse:

DROP WAREHOUSE TOUCAN_WH2;


-- 2.5.21  Show warehouses to verify that TOUCAN_WH is still listed, but
--         TOUCAN_WH2 is not.

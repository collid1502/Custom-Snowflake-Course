...
-- 1.1.10  Name the database TOUCAN_DB.

-- 1.1.11  Create a table.

-- 1.1.12  In the left-side list, click the database you just created.

-- 1.1.13  Click [Create] and create a table named TOUCAN_TBL.
...
DROP DATABASE TOUCAN_DB;

--         NOTE: This will drop any tables in the database, as well. You do not
--         need a warehouse to create objects.

-- 1.2.4   Drop your warehouse, and then recreate your database (TOUCAN_DB)
--         using SQL:

DROP WAREHOUSE TOUCAN_WH;
CREATE DATABASE TOUCAN_DB;


-- 1.2.5   Re-create your table in the PUBLIC schema of your database:

CREATE OR REPLACE TABLE TOUCAN_DB.PUBLIC.TOUCAN_TBL
...
ALTER USER TOUCAN
    ...
    DEFAULT_NAMESPACE=TOUCAN_DB.PUBLIC
    DEFAULT_WAREHOUSE=TOUCAN_WH;

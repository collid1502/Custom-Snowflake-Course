...
-- 3.0.0   Loading and Unloading Structured Data
--         Expect this lab to take approximately 30 minutes.

-- 3.1.0   Create Tables and File Formats
...
-- 3.1.1   Navigate to [Worksheets] and create a new worksheet named Load
...
-- 3.1.2   Execute all of the CREATE TABLE statements:
...
-- 3.1.3   Create a file format called MYPIPEFORMAT, that will read the pipe-
...
-- 3.1.4   Create a file format called MYGZIPPIPEFORMAT that will read the
...
-- 3.2.0   Load the region.tbl File
...
-- 3.2.1   Review the properties of the stage:
...
-- 3.2.2   Confirm the file is in the external stage with the list command:

LIST @training_db.traininglab.ed_stage/load/lab_files/ pattern='.*region.*';


-- 3.2.3   Load the data from the external stage to the REGION table, using the
...
-- 3.2.4   Select and review the data in the REGION table, either by executing
...
-- 3.3.0   Load a GZip Compressed File
...
-- 3.3.1   Empty the REGION Table in the PUBLIC schema of TOUCAN_DB:

TRUNCATE TABLE region;


-- 3.3.2   Confirm that the region.tbl.gz file is in the external stage:

LIST @training_db.traininglab.ed_stage/load/lab_files/ pattern='.*region.*';


-- 3.3.3   Reload the REGION table from the region.tbl.gz file. Review the
...
-- 3.3.4   Query the table to view the data:

SELECT * FROM region;


-- 3.4.0   Load data using the Load Data Wizard
...
-- 3.4.1   First, run the following query to generate the file you’re going to
...
-- 3.4.2   You should see five rows in the results. Click the download button to
--         download the file:
--         Download Button

-- 3.4.3   You will see a dialog box offering you either TSV or CSV format.
--         Select CSV and click Export.

-- 3.4.4   Your file should have been saved to the location you selected.
--         Examine the file using the text editor of your choice. The file
--         should have a header row and five rows of data.

-- 3.4.5   Now let’s create a pipe format using the user interface. In the Web
--         UI, click the Databases button in the ribbon at the top, then click
--         the File Formats tab.

-- 3.4.6   You should see a list of existing file formats, including the ones
--         you created. Click Create.

-- 3.4.7   The Create File Format dialog box should appear. Type MYCSVFORMAT in
...
-- 3.4.8   Your new file format should now be listed. Now click the Tables tab
--         and select the row for table REGION.

-- 3.4.9   Click the Load Table option just above the table list.

-- 3.4.10  You should now see the Load Data dialog box. You will have to select
--         the warehouse, source files, file format and load options. Select
--         TOUCAN_WH and click Next.

-- 3.4.11  You should see an option to load file from your computer. Click the
--         Select Files button, navigate to and select the file you downloaded,
--         then click Next.

-- 3.4.12  You should now see an option to select a file format. Select
--         MYCSVFORMAT and click Next.

-- 3.4.13  You should now see a list of Load Options. These give you different
...
-- 3.4.14  You should see a dialog box saying files are being encrypted and
...
-- 3.4.15  The error should read Numeric value R_REGIONKEY is not recognized.
...
-- 3.4.16  Now go back to your worksheet and run the following SQL statement:

SELECT * FROM region;


-- 3.4.17  You should have five rows in the REGION table now.

-- 3.5.0   Unload a Pipe-Delimited File to a Table Stage

-- 3.5.1   Open a new worksheet and set the context as follows:
...
-- 3.5.2   Create a fresh version of the REGION table with 5 records to unload:

create or replace table REGION as 
select * from SNOWFLAKE_SAMPLE_DATA.TPCH_SF1.REGION;


-- 3.5.3   Unload the data to the REGION table stage. Remember that a table
...
-- 3.5.4   List the stage and verify that the data is there:

LIST @%region;


-- 3.5.5   OPTIONAL: Use GET to download the file to your local system. Open it
...
-- 3.5.6   Remove the file from the REGION table’s stage:

REMOVE @%region;


-- 3.6.0   Use a SQL statement containing a JOIN to Unload a Table into an
--         internal stage

-- 3.6.1   Do a SELECT with a JOIN on the REGION and NATION tables. You can JOIN
...
-- 3.6.2   Create a named stage (you can call it whatever you want):

CREATE OR REPLACE STAGE mystage;


-- 3.6.3   Unload the JOINed data into the stage you created:
...
-- 3.6.4   Verify the file is in the stage:

LIST @mystage;


-- 3.6.5   OPTIONAL: Use GET to download the file to your local system and
...
-- 3.6.6   Remove the file from the stage:

REMOVE @mystage;


-- 3.6.7   Remove the stage:
